...
public interface JobRepository extends CRUDRepository<JobBean, String> {
	...
	@Query(value = "select * from JOB where if(:type != -1, type =:type, 1=1) AND if(COALESCE(:name, '') != '', name like concat('%', :name, '%'), 1=1) AND is_del = 0 order by ctime desc", nativeQuery = true)
	Page<JobBean> pageQuery(Pageable pageable, Integer type, String name);
}
