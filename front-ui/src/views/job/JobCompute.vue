<template>
  <div class="g6-wrap">
    ...
    <a-drawer
      ...
      width="500"
    >
      <div style="margin-bottom: 12px; padding: 8px; background-color: #fffbe6; border: 1px solid #ffe58f; border-radius: 4px;">
        <p style="margin: 0; color: #d48806;">1、当前画布仅支持单SQL算子''</p>
        <p style="margin: 0; color: #d48806;">2、字符串必须使用单引号''</p>
      </div>
      <div>
    ...
    >
      <div style="margin-bottom: 12px; padding: 8px; background-color: #fffbe6; border: 1px solid #ffe58f; border-radius: 4px;">
        <p style="margin: 0; color: #d48806;">1、如果需要引用来源表字段使用${}，例：${division_name}''</p>
      </div>
      <span>大模型prompt</span>
...
export default {
  ...
  mounted () {
    ...
    listQuery().then(res => {
      ...
      const excludeToDs = ['oracle', 'kafka', 'mysqlcdc', 'redis', 'custom', 'es']
      for (var a of record) {
        // redis数据源暂不支持读
      ...
      for (const node of this.graph.getNodes()) {
        ...
        if (node.id === this.currentNodeId) {
          if (node.shape === 'sql') {
            ...
            nodeData['tags'] = this.tags

            for (const i in this.tags) {
              this.mappings.push({ sourceField: this.tags[i] })
            }
          }
          if (node.shape === 'llm') {
